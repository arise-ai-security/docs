...
3. **Better Fixer**: Our system is able to use their patched code against the PoC code and verify if the vulnerbaility is no longer triggered. In contrast, SecVerifier's fixer agent only relies on candidate fixes from external sources (e.g., GitHub commits) without verifying if the patch actually works. In contrast, our system automatically spawn validator agents to verify after fix functionality preservation by using unit tests.
